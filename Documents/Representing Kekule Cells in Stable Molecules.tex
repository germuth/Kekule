...
%Talk about if ports have double bonds like internal nodes do.
...
We choose to ignore Huckel's rule and any extensions. This is because even anti-aromatic compounds such as pentalene can be stabilized by adding the appropriate substituent. Either electron-donating or electron-withdrawing groups can be attached to the ring to allow aromaticity (see Figure ~\ref{fig:pentalene}). Additionally, it is possible to insert hereroatoms into the structure, such as nitrogen and sulphur. In addition to making the molecules easier to synthesize, the nitrogen may add electrons to conjugated system to meet Huckel's rule (see Figure ~\ref{fig:pyrrole}). Although such alterations are not always possible, most graphs can resemble realistic molecules. 
...
\begin{figure}[ht!]
\centering
\includegraphics[width=90mm]{pentalene.png}
\caption{Pyrrole is a heterocyclic aromatic compound, containing a nitrogen. It is considered aromatic due to Huckels Rule (4n + 2, where n = 1}. 4 electrons come from the carbon atoms, two from each double bond, and two additional electrons from nitrogen's lone pair, making 6 total contributing electrons.}
\label{fig:pyrrole}
\end{figure}
