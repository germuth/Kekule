...
Arraylist chanDiv(int k, Intstack set) {
	...
	if (set->size == 0) {
		...
		if (k == 0)
			putItem(newIntstack(5, NULL), result) ;
		return result;
	}
	ch = popint(set) ;
...
int isfan(int p, Intstack fan, Intstack cell) {
	...
	if (fan->size == 0)
		return 1;
	x = popint(fan);
	b = isfan(p, fan, cell) && member(p ^ x, cell) && isfan(p ^ x, fan, cell);
	fan->size++;
	return b;
}
...
int channelConnect(int p, int q, Intstack cell) {
	...
	Intstack chs1;
	Arraylist li;
	int b, i, pp = cell->it[p], dif = pp ^ cell->it[q], cn = cntBits(dif);
	if (cn <= 2)
		return 1;
...
#if 1
	while (i < li->size && !isfan(pp, li->it[i], cell))
		i++;
#endif
	b = (i < li->size);
	for (i = 0; i < li->size; i++) {
