...
Arraylist classify(int r, int raw) {
  ...
  for (x = 0L; x < ulim; x ++) {
    ...
    i = 1;
    while (y > 0) {
      if (y % 2 == 1) putint(a[i], cand);
      y /= 2;
      i++;
    }
    if (isFlexible(ports, cand) 
	&& portHistDescending(cand) 
	&& isCentered(cand) 
	&& isCoherent(cand) ) {
      if (raw) printCell(++nr, cand);
      else putItem((void*) newIntstack(0, cand), result);
    } 
  }
  freestack(cand);
  freestack(ev);
...
void sortAndWeed(int rank, Arraylist ar) {
  ...
  Arraylist variants;
  asort(ar,
	(int(*)(const void*, const void*)) compareCell);
  ...
  while (i < ar->size) {
	...
	if (ar->it[j] != NULL 
	    && compareL(ar->it[j], variants->it[k]) == 0) {
