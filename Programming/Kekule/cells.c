...
int indecomposable(Intstack cell, Intstack part1, Intstack part2) {
	...
	unsigned long long int xx = 0ULL;
	Intstack cand = newIntstack(5, NULL), ncell = newIntstack(cell->size, NULL);
	...
	for (x = 1L; r && x < ulim - 1; x++) {
		...
		while (y > 0) {
			if (y % 2 == 1)
				putint(cell->it[i+1], cand);
			y /= 2;
			i++;
		}
		if (isCoherent(cand) && hasFactor(cell, cand, ncell) ) {
...
int isOriginal(Intstack cell) {
	int i = 0, nf = 1;
	Intstack p1 = newIntstack(cell->size, NULL), p2 = newIntstack(cell->size,
			NULL), other = newIntstack(0, cell);
