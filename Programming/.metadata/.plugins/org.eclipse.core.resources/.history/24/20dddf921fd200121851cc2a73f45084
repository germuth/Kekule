...
void initPerm() {
  ...
  if (permInit) return ;
  permInit = 1 ;
  for (i = 0 ; i < RANKLIM ; i++) perm[i] = NULL ;
...
Arraylist getPerm(int rank) {
  int i, j, k ;
  Intstack y, z ;
  if (! permInit) initPerm() ;
  if (perm[rank] != NULL) return perm[rank] ;
  if (perm[rank-1] == NULL) getPerm(rank-1) ;
  perm[rank] = newArraylist(rank * perm[rank-1]->size) ;
  for (i = 0 ; i < perm[rank-1]->size ; i++) {
    y = perm[rank-1]->it[i] ;
    for (j = 0 ; j < rank ; j++) {
      z = newIntstack(rank, NULL) ;
      for (k = 0 ; k < j ; k++) z->it[k] = y->it[k] ;
      z->it[j] = rank - 1 ;
      for (k = j ; k < y->size ; k++) z->it[k+1] = y->it[k] ;
      z->size = rank ;
      putItem(z, perm[rank]) ;
    }
  }
...
Arraylist specPerm(int rank, int* hist) {
  ...
  while (i + 1 < rank) {
    j = i+1 ;
    while (j < rank && hist[i] == hist[j]) j++ ;
    if (i+1 < j) {
      x = result->size ;
      locperm = getPerm(j-i) ;
      for (k = 0 ; k < locperm->size - 1 ; k++) 
	for (m = 0 ; m < x ; m++) 
	  putItem(applyPerm(i, result->it[m], 
			    locperm->it[k]), result) ;
    }
    i = j ;
  }
...
Arraylist allVariants(int rank, Intstack cell) {
  ...
  for (i = 1 ; i < cen->size ; i++) {
    ...
    if (! memberAr(ce1, result, 
		   (int(*)(const void*, const void*)) compareL)){
#if 0
