...
void borderEdges(int rank, Intstack cell, Intstack result) {
  ...
  for (p = 1; p < lim; p = p << 1) {
	...
	nf = ((ch & cell->it[i]) > 0 
	      || member(ch | cell->it[i], cell));
      }
      if (nf) putint(ch, result);
    }
  }
}
...
int bestBorderGraph(int rank, Intstack cell) {
  Intstack cpCell = newIntstack(cell->size, NULL),
    borderedges = newIntstack(0, NULL);
  int i, x = 0, xsize = -1, y;
...
Intstack potEdges(int rank, int cn) {
  int size = rank * (cn - rank), p, q
    , pend = 1 << rank, qend = 1 << cn;
...
Graph findGraphR(Graph g0, Intstack c0, Intstack cell, 
		 Intstack ledges, int lptr) {
  int ed, uu, edsize;
  Intstack kk, c1;
  Graph result;
  if (c0->size == cell->size) return g0;
  if (lptr == ledges->size) return NULL;
